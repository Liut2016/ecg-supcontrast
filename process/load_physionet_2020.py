...
# 使用Challenge2020中的CPSC数据
# 处理后的数据（frame）为dict，格式为{patientid:data}，共6877条数据，其中data格式为[12, 2500],12代表（4导联 * 3段），每段2500个采样点
# 处理后的标签（label）为dict，格式为{patientid:label}，共6877条数据，其中label格式为[12, 9], 为one-hot形式


# %%
# 改成数据存储地址
#basepath = '../data/raw_data/PhysioNet2020/Training_CPSC/Training_WFDB'
basepath = '/home/liutao/ecg-data/PhysioNet2020/Training_CPSC'
...
def encode_outputs(all_labels, outputs):
    ...
    for file, values in outputs.items():
        ...
        indices = [[enc.transform([el]).item() for el in value] for value in
                   values]  # convert arrhythmia label to number
...
def makepath_and_save_data(inputs, outputs, leads_of_interest, trial=''):
    # 改成处理后数据的存储地址
    basepath = '../data/PhysioNetChallenge2020_Training_CPSC/Training_WFDB'
...
# %%
leads_of_interest = [['II','V2','aVL','aVR']]
# [['II','V2','aVL','aVR']] #[['I','II','III','aVL','aVR','aVF','V1','V2','V3','V4','V5','V6']] #list of lists regardless of number of leads
